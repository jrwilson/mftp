...
namespace jam {
  ...
  public:
	...
	if (sender->get_handle () != -1 && converter->get_handle () != -1) {
	  ...
	  new ioa::automaton_manager<mftp::mftp_automaton> (this, ioa::make_generator<mftp::mftp_automaton> (file, sender->get_handle(), converter->get_handle(), false));
	
	  // Create the meta server.
	  new ioa::automaton_manager<mftp::mftp_automaton> (this, ioa::make_generator<mftp::mftp_automaton> (meta, sender->get_handle (), converter->get_handle (), query_predicate (), query_filename_predicate (m_sharename), false, false));
