...
namespace jam {
  ...
  private:
	...
	if (channel->get_handle () != -1) {
	  ...
	  ioa::automaton_manager<mftp::mftp_automaton>* query = new ioa::automaton_manager<mftp::mftp_automaton> (this, ioa::make_generator<mftp::mftp_automaton> (f, channel->get_handle (), meta_inst_predicate (m_filename), meta_inst_filename_predicate (m_filename), true, false, 0));
    ...
    void match_complete_effect (const ioa::const_shared_ptr<mftp::file>& meta_file) {
      if (meta_file->get_mfileid ().get_fileid ().type == META_TYPE) {
