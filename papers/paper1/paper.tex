...
There are three types of messages in the protocol, the first of which being a \emph{request}.
A \emph{request} contains a file ID and a set of fragment indices.
...
\paragraph {Tasks.}
MFTP is a peer-to-peer protocol since all participants can act as both a client and a server.
In a simple scenario, if a peer gets a request for a file it has fragments for, it can send those fragments out.
