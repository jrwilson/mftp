...
\paragraph{File ID's.}
A structure that contains a hash, length, and type. Each file ID is unique to its file.

\paragraph{Matching.}
Given a file $f_1$ and $f_2$, one can define a matching predicate $\mu$.

\paragraph{Message types.}
Request: a file ID and a set of fragment indices. We implemented requests as the file ID and a set of contiguous regions.  However, one can envision the request to contain anything that defines a set.
Fragment: a file ID, fragment index, and the data itself.
Match: a file ID and a set of matching files.
\paragraph{How it's put together.} %% change
